...
- **모든 데모 페이지는 `demo/` 폴더 안에 생성**한다 (`demo/YYYYMMDD-기능명-demo.html`)
- 데모 페이지의 UI는 **현재 앱 UI와 동일**하게 제작한다
  - 색상: 앱과 동일한 색상 팔레트 사용 (`#2563eb`, `#172f5a`, `#f0f4f8` 등)
  - 폰트: Pretendard 패밀리만 사용
  - 컴포넌트 스타일: 버튼·배지·카드·테이블 등 앱과 동일한 형태
- 앱 UI가 변경되면 데모 페이지도 함께 업데이트한다
...
15. **데모 페이지**: `demo/` 폴더에 생성, 현재 앱 UI(색상·폰트·컴포넌트 스타일)와 동일하게 제작
