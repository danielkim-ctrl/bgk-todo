...
| **데모 페이지 생성** | `demo/*.html` 파일 Write | 14(데모 UI 일치) |
...
**데모 페이지는 반드시 현재 앱 UI와 동일하게 제작한다.**

데모 페이지가 앱과 다른 스타일이면 실제 앱 품질을 오해하게 만든다.

### 필수 규칙
- **위치**: 모든 데모 페이지는 `demo/` 폴더 안에 생성한다 (`demo/*.html`)
- **색상**: 앱과 동일한 색상 팔레트 사용
  - Primary: `#2563eb` / Header: `#172f5a` / Background: `#f0f4f8` / Surface: `#fff`
  - Border: `#e2e8f0` / Text: `#1a2332` / Muted: `#64748b`
- **폰트**: `'Pretendard', system-ui, sans-serif` — 다른 폰트 패밀리 사용 금지
- **컴포넌트 스타일**: 버튼·배지·카드·테이블 등 앱과 동일한 형태·크기·반경 적용
- **앱 UI 변경 시**: 관련 데모 페이지도 함께 업데이트

### 위반 패턴

```html
<!-- ❌ 앱과 다른 색상 사용 -->
<button style="background: #007bff">저장</button>

<!-- ✅ 앱과 동일한 색상 사용 -->
<button style="background: #2563eb; border-radius: 8px; font-family: 'Pretendard', system-ui, sans-serif">저장</button>
```
...
| "데모 페이지를 어디에 만들어야 하나?" | **#14 데모 UI 일치** |
| "데모 페이지 색상/폰트를 뭘 써야 하나?" | **#14 데모 UI 일치** |
